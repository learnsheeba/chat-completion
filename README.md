...
1. Assuming github repo is created and clone url is available.
1. In command prompt `git clone <github-url>`
1. `cd <github-repo-name>`
...
1. `pip freeze > requirements.txt` - all installed package names stored here, later/in future helps to install packages in one line command `pip install -r requirements.txt`
1. create a file name `.env` and add the contents from `.envExample`
